...
var show_binary_of_heap = function(){
   vartotal = [];
  $('.heaps').each(function(){
    var total_unselected = 0;
    temp = $(this).find('.item').filter("[is_selected='0']").length;
    if(temp > 0){
      total_unselected = temp;
    }
    binary_value =  dec_to_bho(total_unselected,'B');
    var id_attr = $(this).attr('id');
    vartotal.push(binary_value);
    $('#'+id_attr+'-binary').html(binary_value);
    console.log(vartotal);
  })
  sum_heap = vartotal['0']^vartotal['1']^vartotal['2'];
  $('#nim_sum').html(sum_heap);
}
...
var initGame = function() {
  ...
  if($("input[name='player-one']").val()) {
    ...
    $('#player-one').text("");
    $('#player-one').append("<i class='material-icons'>person</i> ");
    $('#player-one').append(playerOneName);
  }
  if(aiMode === true) {
    $('#player-two').text("");
    ...
    if($("input[name='player-two']").val()){
      ...
      $('#player-two').text("");
      $('#player-two').append("<i class='material-icons'>person</i> ");
      $('#player-two').append(playerTwoName);
    }
  }
};
  ...
  setTimeout(function () {
    ...
    setTimeout(function() {
      ...
      if(player === 1 || player === 2 && aiMode === false) {
        ...
        $('.heap-one').append("<img src='assets/images/ai-brain.png' class='brain responsive-img'>");
        $('.heap-two').append("<h3 class='win-msg'>The Computer beat you...</h3>");
      }
...
var aiPlayTurn = function() {
  ...
  while(quantityRemoved < quantityToRemove) {
    ...
    if($(idString).attr("is_selected") != '1'){
  ...
  //computer is done with it now turn
  if(!gameOver) {
    player = 1;
    $('#player-two').addClass('disabled');
    $('#player-one').removeClass('disabled');
...
function recalulated_binary_of_heap(heap_number){

}
var removeItem = function() {

  is_already_selected = $(this).attr('is_selected');
  if(is_already_selected == 1){
    M.toast({html: 'This match stick already selected!', classes: 'rounded'});
    return;
  }
  ...
  if($(this).parent().attr('id') === selectedHeap) {
    ...
    recalulated_binary_of_heap(selectedHeap);
    //$(this).hide();
    $(this).css("background-image", "url(assets/images/burned_matchstick.png)");
